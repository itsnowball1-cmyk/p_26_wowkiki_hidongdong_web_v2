...
export default function FAQEditForm({ idx, onBack }: { idx: number; onBack: (saved?: boolean) => void }) {
  ...
  useEffect(() => {
    fetch(`/api/admin/faq/${idx}`, { headers: HEADERS })
      .then(r => r.ok ? r.json() : null)
      .then((d: { status: string; target_roles: string; category: string; title: string; content: string } | null) => {
        if (!d) return
        let detail = '', answer = ''
        try {
  ...
  const handleSubmit = async () => {
    ...
    setSaving(true)
    const res = await fetch(`/api/admin/faq/${idx}`, {
      method: 'PUT',
      headers: HEADERS,
      body: JSON.stringify({
        status:       form.status,
  ...
  return (
    <Layout title="FAQ">
      ...
      <h1 className="text-[30px] font-semibold text-[#000000] mb-6">FAQ 수정</h1>
