...
export default function SupportList() {
        ...
        <table className="w-full text-[14px]">
          ...
          <tbody className="divide-y divide-line">
            {loading && Array.from({ length: 3 }).map((_, i) => (
              <tr key={i}><td colSpan={6} className="py-4 px-4"><div className="h-4 rounded animate-pulse bg-line w-full" /></td></tr>
            ))}
            {!loading && items.length === 0 && (
              <tr><td colSpan={6} className="py-16 text-center text-ink-400">등록된 문의가 없습니다.</td></tr>
            )}
            {!loading && items.map((item, i) => (
              <tr key={item.cs_idx} onClick={() => go({ name: 'support-detail', id: item.cs_idx })}
  ...
  if (isRestricted) {
    return <RestrictedLayout onBack={() => go({ name: 'list' })}>{content}</RestrictedLayout>
  }
